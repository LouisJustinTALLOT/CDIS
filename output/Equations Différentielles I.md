...
de classe $C^p$ vérifie $$
y^{(p)}(t) = \psi\left(t,y(t),\dot{y}(t),\ldots, y^{(p-1)}(t)\right)
$$ si et seulement si $x=(y,\dot{y},\ldots,y^{(p-1)})$ de classe $C^1$
vérifie $$
\dot{x} = f(t,x) \ , 
...
$X = \mathbb{R}^{n}$ entiers. Cependant, il peut arriver que cela ne
soit pas le cas, comme par exemple pour deux corps de position $y_a,y_b$
dont la force d'intéraction gravitationnelle
$\frac{Gm_a m_b}{\|y_a-y_b\|^2}$ n'est définie que pour $y_a\neq y_b$.
:::
...
est dûe à Cauchy et porte le nom de *Problème de Cauchy* :
...
plus général consisterait à autoriser des discontinuités de $f$ en $x$
mais l'étude des solutions passerait alors par celle des *inclusions
...
existent toujours si $f$ est continue. Nous savons qu'elles sont
définies *au moins un certain temps*, mais il est intéressant de se
demander quel est l'intervalle de temps *maximal* sur lequel elles
peuvent être définies. En d'autre terme, on s'intéresse aux *solutions
maximales*.
...
elle ne peut être prolongée au delà. On dit qu'elle *explose en temps
fini*.
...
définies sur un intervalle ouvert, et cet intervalle ne peut être borné
seulement si $t\mapsto x(t)$ diverge en temps fini ou $t\mapsto(t,x(t))$
tend vers la frontière de l'ensemble de définition $J\times X$ de $f$.
:::
...
$t_m^-,t_m^+\in \mathbb{R}\cup\{+\infty,-\infty\}$. De plus, $$
\lim_{t\to t_m^-} d\Big((t,x(t)),\partial (J\times X) \Big) = 0  \quad  \text{ou} \quad 
\lim_{t\to t_m^-} \|(t,x(t))\| = +\infty 
$$ et $$
\lim_{t\to t_m^+} d\Big((t,x(t)),\partial (J\times X) \Big) = 0  \quad  \text{ou} \quad 
\lim_{t\to t_m^+} \|(t,x(t))\| = +\infty  \ .
$$
...
Si l'intervalle d'existence n'était pas ouvert, la solution pourrait
être prolongée au bord grâce au théorème de Peano, ce qui contredirait
sa maximalité. Ensuite, la preuve consiste à montrer que si $t_m^+$
(resp. $t_m^-$) est fini, alors $(t,x(t))$ finit forcément par sortir
définitivement de tout sous-ensemble fermé et borné de $J\times X$
lorsque $t$ tend vers $t_m^+$ (resp. $t_m^-$). Voir en [annexe (p.
...
solutions est garantie si $f$ est de plus continûment différentiable par
rapport à la variable $x$. On voit que ce n'est pas le cas de
...
hypothèse). Alors l'application du théorème des accroissement finis nous
donne $$
...
solution maximale $x_\delta$ dans $S_f(t_0,x_0+\delta)$ est définie sur
...
$f:(x_1,x_2)\mapsto (\sin x_1 - x_2 ,\sqrt{1+x_1^2})$ est continûment
différentiable sur $\mathbb{R}^2$. Donc d'après le théorème de
Cauchy-Lipschitz, le problème de Cauchy admet une unique solution
maximale définie sur un intervalle de temps $I$ ouvert.
...
Cauchy-Lipschitz, les solutions sont uniques. De plus, la dynamique est
linéaire (donc a fortiori linéairement bornée) donc les solutions sont
définies pour tout $t$. Les solutions sont données par $x(t)=e^{At}x_0$.
:::
...
Or, vu que $f$ est définie sur $\mathbb{R}^n$, on sait du [Théorème du
domaine maximal d'existence (p.
`\pageref*{theo_bouts}`{=tex})](#theo_bouts) que la seule raison pour
laquelle une solution maximale ne serait pas définie pour tout $t\geq 0$
serait qu'elle explose en temps fini. C'est impossible ici.
:::
